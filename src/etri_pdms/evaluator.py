...
def evaluate_sample(token,plans,dataset,out,cfg):
    ...
    try:
        ...
        row['evaluation_stage']='gt_mpc'
        gt_roll=rollout(gt_ref,sample['initial'],cfg)
        row['evaluation_stage']='prediction_mpc'
        model_roll=rollout(pred_ref,sample['initial'],cfg)
    ...
    if row['valid']:row['evaluation_stage']='complete'
    write_json(folder/'result.json',row)
    return row

def evaluate(pred_path,infos,root,out,cfg,limit=None,tokens=None,visualize=20,workers=1,sample_interval=0.0):
    ...
    if not np.isfinite(sample_interval) or sample_interval<0: raise ValueError('sample_interval must be finite and nonnegative')
    if tokens is not None and sample_interval>0: raise ValueError('Exact --tokens requires --sample-interval 0')
    out=Path(out)
    ...
    from .parallel import run_scenes
